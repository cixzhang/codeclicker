...
CodeClicker.initRender = function() {
  ...
  Object.keys(CodeClicker.bugMap)
    .forEach(function (bug) {
      ...
      var onClick = function () {
        CodeClicker.programmer.fixBug(bug);
        CodeClicker.data.showDoc = false;
      };
  ...
  CodeClicker.render = function (time) {
    ...
    var typeSpeed = Math.floor(Math.max(CodeClicker.data.msPerAction, 10) / 5);
  ...
  var titleText = new PIXI.Text('Code Clicker', {
    fontFamily: 'VT323',
    fontSize: 26,
    fill: bg,
    align: 'center'
  });
  var crashText = new PIXI.Text('Booting...', {
    fontFamily: 'VT323',
    fontSize: 22,
    fill: bg,
    align: 'left'
  ...
  titleText.position.set(0.25 * width, 0.75 * height - 100);
  crashText.position.set(0.25 * width, 0.75 * height - 50);
  ...
  CodeClicker.renderCrashSequence = function (time) {
    ...
    var progress = (Math.min(deltaTime, MAX_TIME) / MAX_TIME) * (0.5 * width - 6);
    crashProgress.width = Math.floor(progress);
    crashProgress.x = 0.5 * width - 6 - progress;

    if (deltaTime > 1000) {
      crashStage.addChild(titleText);
    }

    var visibleText = Math.floor(time / 600) % 2;
    if (deltaTime > 1500) {
      crashStage.addChild(crashText);
      crashText.visible = Boolean(visibleText);
    }
    ...
    if (deltaTime > MAX_TIME + 500) {
      ...
      CodeClicker.Sounds.on.volume = 0.4;
      CodeClicker.Sounds.on.play();
    }
