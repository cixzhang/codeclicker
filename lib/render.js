...
CodeClicker.initRender = function() {
  ...
  CodeClicker.render = function (time) {
    ...
    currentUpgrades.forEach(function (upgrade, i) {
      upgrades.addChild(upgradeSprites[upgrade.id]);
    });
    availableUpgrades.forEach(function (upgrade, i) {
      available.addChild(upgradeSprites[upgrade.id]);
    });
