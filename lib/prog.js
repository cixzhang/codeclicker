...
function apply(numModules, bugFree) {
  ...
  if (bugged) CodeClicker.Sounds.bug.play();
  else CodeClicker.Sounds.fix.play();
  return bugged ? 'Oops...Guess I should have tested that...' : null;
}

function applyBug(bug) {
  if (!bug) return;
  if (CodeClicker.bugs[bug]) return;
...
function fixBug(bug) {
  ...
  if (CodeClicker.bugs[bug] < 1) {
    CodeClicker.Sounds.fix.play();
    info.unvoke();
  }
}
